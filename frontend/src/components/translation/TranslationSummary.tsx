...
export function TranslationSummary({ translationResult, onComplete, onBack }: TranslationSummaryProps) {
  ...
  const processingTime = result?.processing_time != null ? `${result.processing_time}s` : '—';
  const stats = result?.statistics || {};
  const sectionsCount = stats.sections_translated ?? result?.translated_sections?.length ?? 0;
  ...
  // Confidence metrics  
  const overallConfidence = result?.overall_confidence != null ? result.overall_confidence * 100 : 0;
  const legalTermAccuracy = stats.glossary_match_rate != null ? stats.glossary_match_rate * 100 : 0;
          ...
          <CardContent className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                ...
                <p className="font-medium text-foreground font-mono text-xs">{jobId || '—'}</p>
              </div>
              <div className="space-y-1">
            ...
            <div className="grid grid-cols-4 gap-4 pt-4 border-t border-border">
              <div className="text-center p-4 bg-muted rounded-lg">
                <p className="text-2xl font-bold text-foreground">{sectionsCount}</p>
                <p className="text-xs text-muted-foreground mt-1">Sections Translated</p>
              </div>
              <div className="text-center p-4 bg-muted rounded-lg">
              ...
              <div className="text-center p-4 bg-muted rounded-lg">
                <p className="text-2xl font-bold text-foreground">{legalTermsCount}</p>
              ...
              <div className="space-y-3">
                <div>
                  <div className="flex items-center justify-between text-sm mb-1">
                    ...
                    <span className="font-medium text-success">{overallConfidence.toFixed(1)}%</span>
                  </div>
                ...
                <div>
                  <div className="flex items-center justify-between text-sm mb-1">
                    ...
                    <span className="font-medium text-success">{legalTermAccuracy.toFixed(1)}%</span>
                  </div>
                ...
                <div>
                  <div className="flex items-center justify-between text-sm mb-1">
                    ...
                    <span className="font-medium text-foreground">{bleuScore.toFixed(3)}</span>
                  </div>
        ...
        <div className="space-y-4">
          <Card>
            ...
            <CardContent className="space-y-3">
              <Button variant="outline" className="w-full justify-start gap-3" onClick={() => handleExport('pdf')}>
              ...
              <Button variant="outline" className="w-full justify-start gap-3" onClick={() => handleExport('txt')}>
                ...
                <div className="text-left">
                  ...
                  <p className="text-xs text-muted-foreground">Plain text TXT format</p>
                </div>
              </Button>
              
              <Button variant="outline" className="w-full justify-start gap-3" onClick={() => handleExport('json')}>
