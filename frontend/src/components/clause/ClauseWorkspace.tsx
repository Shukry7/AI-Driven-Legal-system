...
export function ClauseWorkspace({
  file,
  onComplete,
  onCancel,
  originalDocument,
}: ClauseWorkspaceProps) {
...
Judge: [MISSING: Third Judge Signature - Signature required]
  ...
  const [documentText, setDocumentText] = useState<string>(
    originalDocument ?? defaultDocumentText,
  );
  ...
  const [corruptedRegions, setCorruptedRegions] = useState<
    Array<{ clause_name: string; text: string; start: number; end: number }>
  >([]);
  const [editingClauseId, setEditingClauseId] = useState<number | null>(null);
  const [editedText, setEditedText] = useState("");
  const [activePopover, setActivePopover] = useState<string | null>(null);
  const [modifiedDocumentText, setModifiedDocumentText] =
    useState(documentText);
  const [savedTextFilename, setSavedTextFilename] = useState<string | null>(
    null,
  );
  ...
  // ─── AI Prediction State ─────────────────────────────────────────
  const [predictionConfig, setPredictionConfig] =
    useState<PredictionConfig | null>(null);
  const [predictions, setPredictions] = useState<PredictionResult | null>(null);
  const [predictionsLoading, setPredictionsLoading] = useState(false);
  const [predictionsError, setPredictionsError] = useState<string | null>(null);
  const [suggestionStatuses, setSuggestionStatuses] = useState<
    Record<string, "pending" | "accepted" | "edited" | "rejected">
  >({});
  const [editingSuggestionKey, setEditingSuggestionKey] = useState<
    string | null
  >(null);
  ...
  const steps: ProcessStep[] = [
    ...
    {
      id: 5,
      name: "AI is Generating Suggestions...",
      status: "pending",
      icon: "🧠",
    },
  ];
    ...
    corruptedClauses: [
      {
        ...
        section: "S.C. Case No:",
        suggestion:
          "Verify the complete SC case number from court records. The corrupted text shows: SC/###/LA 184/2023",
      ...
      },
      {
        ...
        suggestion:
          "The date appears to be in March 2023 - verify the exact day. The corrupted text shows: ##.03.2023",
  ...
  const startAnalysis = async () => {
    ...
    try {
      ...
      const resp = await api.analyzeClauses(file, (p: number) =>
        setProgress(Math.max(8, Math.round(p * 0.9))),
      );
      ...
      if (resp && resp.success) {
        ...
        if (
          resp &&
          resp.clause_analysis &&
          Array.isArray(resp.clause_analysis.corrupted_regions)
        ) {
          ...
          setTimeout(() => {
            ...
            if (resp.predictions && resp.prediction_mode === "auto") {
              ...
              if (resp.predictions.suggestions) {
                Object.keys(resp.predictions.suggestions).forEach((k) => {
                  statuses[k] = "pending";
                });
              }
  ...
  const handleGetAISuggestions = async (forceRefresh: boolean = false) => {
    if (!savedTextFilename) {
      setPredictionsError(
        "No saved text file available. Please run analysis first.",
      );
      return;
    }
    ...
    try {
      ...
      if (result.suggestions) {
        Object.keys(result.suggestions).forEach((k) => {
          statuses[k] = "pending";
        });
      }
      setSuggestionStatuses(statuses);
  ...
  const handleAcceptSuggestion = async (clauseKey: string) => {
    ...
    try {
      ...
      const formattedText = formatClauseForInsertion(
        clauseKey,
        suggestion.clause_name,
      ...
      if (clauseKey === "judge_concurrence") {
        ...
        console.log(
          "📍 Judge concurrence: forcing insertion at END of document (position:",
          insertPosition,
          ")",
        );
      } else {
      ...
      const updatedDoc =
        modifiedDocumentText.substring(0, insertPosition) +
        formattedText +
        modifiedDocumentText.substring(insertPosition);
  ...
  const handleSaveSuggestionEdit = async (clauseKey: string) => {
    ...
    try {
      ...
      if (clauseKey === "judge_concurrence") {
        insertPosition = modifiedDocumentText.length;
        console.log(
          "📍 Judge concurrence (edited): forcing insertion at END of document (position:",
          insertPosition,
          ")",
        );
      } else {
      ...
      alert(
        "Failed to save edited suggestion: " + (err.message || "Unknown error"),
      );
    }
  };
  ...
  const getPredictionStats = () => {
    ...
    const accepted = Object.values(suggestionStatuses).filter(
      (s) => s === "accepted",
    ).length;
    const rejected = Object.values(suggestionStatuses).filter(
      (s) => s === "rejected",
    ).length;
    const edited = Object.values(suggestionStatuses).filter(
      (s) => s === "edited",
    ).length;
    const pending = Object.values(suggestionStatuses).filter(
      (s) => s === "pending",
    ).length;
  ...
  const formatClauseForInsertion = (
    clauseKey: string,
    clauseName: string,
    text: string,
  ): string => {
  ...
  const findInsertionPositionFromAnchor = (
    ...
    if (!anchorText || anchorText.trim().length === 0) {
      console.warn(
        "⚠️ No anchor text provided, defaulting to MIDDLE of document",
      );
    ...
    const normalizedIndex = normalizeWhitespace(cleanDoc.toLowerCase()).indexOf(
      normalizedAnchor.toLowerCase(),
    );
    if (normalizedIndex !== -1) {
      console.log(
        "✅ Found normalized match at approximate position:",
        normalizedIndex,
      );
      // Approximate position
      return position === "before"
        ? normalizedIndex
        : normalizedIndex + normalizedAnchor.length;
    }
    ...
    if (anchorText.length > 30) {
      ...
      if (anchorIndex !== -1) {
        console.warn(
          "⚠️ Found PARTIAL match (first 30 chars) at position:",
          anchorIndex,
        );
        return position === "before"
          ? findOriginalPosition(documentText, cleanDoc, anchorIndex)
          : findOriginalPosition(
              documentText,
              cleanDoc,
              anchorIndex + partialAnchor.length,
            );
      }
    }
  ...
  const findClauseInsertionPosition = (
    text: string,
    clauseKey: string,
  ): number => {
    ...
    if (["date_of_order", "hearing_dates"].includes(clauseKey)) {
      const caseMatch = text.search(
        /Case\s+(?:No\.|Number)|Criminal\s+Appeal|Civil\s+Appeal|Writ\s+Petition/i,
      );
      if (caseMatch !== -1) {
    ...
    if (clauseKey === "referred_cases") {
      ...
      const citationMatch = text.search(
        /(AIR|SCC|\d{4}\s+\(\d+\)|referred to|relied upon|cited)/i,
      );
      if (citationMatch !== -1) {
  ...
  const generateReport = () => {
    ...
    const acceptedCount = [
      ...results.missingClauses,
      ...results.corruptedClauses,
    ...
    const editedCount = [
      ...results.missingClauses,
      ...results.corruptedClauses,
    ].filter((c) => c.status === "edited").length;
...
${c.status === "accepted" ? `Accepted Text: ${c.userInputValue || c.predictedText}` : ""}`,
  )
  .join("\n")}
...
Status: ${c.status || "Pending"}
Issue: ${c.issue}
${c.status === "accepted" ? `Corrected Text: ${c.userInputValue || c.predictedText}` : ""}`,
  )
  .join("\n")}
...
--- END OF REPORT ---`;
  ...
  const handleAcceptClause = (type: "missing" | "corrupted", id: number) => {
    ...
    if (clause) {
      ...
      if (type === "corrupted") {
        ...
        const valueToUse =
          corruptedClause.userInputValue || corruptedClause.predictedText;
        updatedDoc = updatedDoc.replace(/\[CORRUPTED: [^\]]+\]/, valueToUse);
      } else {
        ...
        if (markerIndex !== -1) {
          ...
          const valueToUse =
            missingClause.userInputValue || missingClause.predictedText;
          ...
          updatedDoc =
            updatedDoc.substring(0, lineStart) +
            replacement +
            updatedDoc.substring(lineEnd + 1);
        }
      }
  ...
  const handleSaveEdit = (type: "missing" | "corrupted", id: number) => {
    ...
    if (clause) {
      ...
      if (type === "corrupted") {
        ...
        updatedDoc = updatedDoc.replace(/\[CORRUPTED: [^\]]+\]/, editedText);
      } else {
  ...
  const handleDoneDocumentEdit = async () => {
    ...
    try {
      setAnalyzing(true);
      setProgress(75);
      const resp = await api.saveTextFile(
        savedTextFilename,
        modifiedDocumentText,
      );
      if (resp && resp.success) {
  ...
  const renderDocumentWithHighlights = () => {
    ...
    return lines.map((line, idx) => {
      ...
      const lineCorruptedRegions = corruptedRegions.filter(
        (region) =>
      ...
      const heuristicMatch = corruptedMatches.find(
        (m) => m && line.includes(m) && /[^\w\s-]/.test(m),
      );
      ...
      if (isCorrupted) {
        ...
        const clause =
          results?.corruptedClauses.find(
            (c) =>
              line.toLowerCase().includes(c.section.toLowerCase()) ||
              (corruptedText.includes("SC/") &&
                c.name.includes("Case Number")) ||
        ...
        return (
          <div key={idx} className="hover:bg-warning/5 transition-colors">
            {beforeText}
            <Popover
              open={activePopover === `corrupted-${clause?.id}`}
              onOpenChange={(open) =>
                setActivePopover(open ? `corrupted-${clause?.id}` : null)
              }
            >
      ...
      if (isMissing) {
        ...
        const clause = results?.missingClauses.find(
          (c) =>
        ...
        return (
          <div key={idx} className="my-4">
            <Popover
              open={activePopover === `missing-${clause?.id}`}
              onOpenChange={(open) =>
                setActivePopover(open ? `missing-${clause?.id}` : null)
              }
            >
              <PopoverTrigger asChild>
                <div className="relative bg-destructive/5 border-l-4 border-destructive p-4 rounded-r-lg cursor-pointer hover:bg-destructive/10 transition-all duration-200 group">
      ...
      if (lineCorruptedRegions.length > 0) {
        const sortedRegions = [...lineCorruptedRegions].sort(
          (a, b) => b.start - a.start,
        );
        ...
        for (const region of sortedRegions) {
          ...
          if (
            ...
            while (
              (match = corruptionPattern.exec(corruptedSection)) !== null
            ) {
              // Add text before the corruption
              if (match.index > lastIndex) {
                parts.push({
                  text: corruptedSection.substring(lastIndex, match.index),
                  isCorrupted: false,
                });
              }
            ...
            if (lastIndex < corruptedSection.length) {
              parts.push({
                text: corruptedSection.substring(lastIndex),
                isCorrupted: false,
              });
            }
      ...
      if (heuristicMatch) {
        ...
        const afterText = line.substring(
          line.indexOf(heuristicMatch) + heuristicMatch.length,
        );
        return (
          <div key={idx} className="hover:bg-warning/5 transition-colors">
            {beforeText}
            <span
              className="relative inline-block bg-warning/60 text-warning-foreground px-2 py-0.5 rounded cursor-help border border-warning/50"
              title="Corruption detected by heuristics"
            >
              {heuristicMatch}
            </span>
            {afterText}
      ...
      return (
        <div key={idx} style={{ whiteSpace: "pre", wordWrap: "break-word" }}>
          {line || "\u00A0"}
        </div>
      );
    });
        ...
        {isEditing ? (
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              <Button
                size="sm"
                onClick={() => handleSaveEdit(type, clause.id)}
                ...
                <div className="space-y-2">
                  ...
                  {clause.inputType === "signature" ? (
                    <Textarea
                      id={`input-${clause.id}`}
                      placeholder={
                        clause.placeholderText || "Type your signature"
                      }
                    ...
                    <Input
                      id={`input-${clause.id}`}
                      type={
                        clause.inputType === "date"
                          ? "date"
                          : clause.inputType === "number"
                            ? "number"
                            : "text"
                      }
  ...
  const handleDownloadDocument = async () => {
    try {
      if (!savedTextFilename) {
        alert(
          "No file available for download. Please upload a document first.",
        );
        return;
      }
      ...
      const response = await fetch(buildApiUrl("finalize-and-download-pdf"), {
        method: "POST",
        body: formData,
      });
  ...
  const handleSaveToDatabase = async () => {
    try {
      ...
      const finalizedFilename = savedTextFilename.replace(
        ".clean.txt",
        "_finalized.clean.txt",
      );
  ...
  const getOrganizedClauses = () => {
    ...
    if (
      resp &&
      resp.clause_analysis &&
      Array.isArray(resp.clause_analysis.clauses)
    ) {
      const clauses = resp.clause_analysis.clauses;
      return {
        present: clauses
          .filter((c: any) => c.status === "Present")
          .map((c: any) => c.clause_name),
        missing: clauses
          .filter((c: any) => c.status === "Missing")
          .map((c: any) => c.clause_name),
        corrupted: clauses
          .filter((c: any) => c.status === "Corrupted")
          .map((c: any) => c.clause_name),
      };
    }
  ...
  const renderClauseDetailsDialog = () => {
    ...
    const presentCount =
      results?.statistics?.present || results?.presentClauses || 0;
    const missingCount =
      results?.statistics?.missing || results?.missingClauses.length || 0;
    const corruptedCount =
      results?.statistics?.corrupted || results?.corruptedClauses.length || 0;

    return (
      <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col gap-0 p-0">
        <div className="px-6 py-4 border-b flex-shrink-0">
          <DialogTitle className="text-xl font-bold">
            Clause Detection Details
          </DialogTitle>
          <DialogDescription>
            Complete breakdown of all{" "}
            {results?.statistics?.total_clauses || results?.totalClauses || 0}{" "}
            clauses analyzed in this document
          </DialogDescription>
        </div>
              ...
              <TabsContent value="missing" className="space-y-2 m-0">
                {organized.missing.length > 0 ? (
                  ...
                  <div className="text-center py-8 text-muted-foreground">
                    <Info className="w-8 h-8 mx-auto mb-2 opacity-50" />
                    <p className="text-sm">
                      No missing clauses - document is complete!
                    </p>
                  </div>
                )}
              </TabsContent>
  ...
  const renderComparisonView = () => {
    ...
    const originalLines = stripFormattingMarkers(documentText)
      .trim()
      .split("\n");
    const modifiedLines = stripFormattingMarkers(modifiedDocumentText)
      .trim()
      .split("\n");
    ...
    return (
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <div
            className="font-mono text-xs text-foreground bg-muted/30 p-2 rounded-lg border border-border h-[600px] overflow-y-auto"
                ...
                className={`${
                  line.includes("[CORRUPTED:")
                    ...
                    : line.includes("[MISSING:")
                      ? "bg-destructive/10 border-l-2 border-destructive px-2"
                      : ""
                }`}
        ...
        <div>
          ...
          <div
            className="font-mono text-xs text-foreground bg-success/5 p-2 rounded-lg border border-success h-[600px] overflow-y-auto"
            style={{ lineHeight: "1.2" }}
          >
                ...
                <PopoverContent className="w-80">
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Our AI analyzes your legal document to identify clauses,
                      detect issues, and automatically suggest fixes for missing
              ...
              <div className="text-center space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground max-w-md mx-auto">
                    Click "Start AI Analysis" to begin. The AI will extract
                    text, identify clauses, validate structure, and
                    automatically suggest fixes for any issues.
                  </p>
                </div>
              ...
              <div className="flex gap-3 py-2 flex-nowrap">
                {processSteps.map((step) => (
                  <div
                    ...
                    style={{
                      flex: `1 1 ${100 / processSteps.length}%`,
                      minWidth: 0,
                    }}
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div
                        className={`text-sm font-medium ${step.status === "complete" ? "text-foreground" : "text-muted-foreground"}`}
                      >
                        {step.name}
                      </div>
          ...
          <>
            <Card>
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      Review the findings below and accept or edit AI
                      suggestions
                    </p>
                  </div>
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                  <div className="flex flex-col items-center justify-center p-4 bg-muted rounded-lg">
                    <span className="text-3xl font-bold text-foreground">
                      {results.statistics?.total_clauses ||
                        results.totalClauses}
                    </span>
                  ...
                  <div className="flex flex-col items-center justify-center p-4 bg-success/10 rounded-lg">
                    <span className="text-3xl font-bold text-success">
                      {results.statistics?.present || results.presentClauses}
                    </span>
                    <span className="text-sm font-semibold text-success mt-1">
                      Present
                    </span>
                  </div>
                  <div className="flex flex-col items-center justify-center p-4 bg-destructive/10 rounded-lg">
                    <span className="text-3xl font-bold text-destructive">
                      {results.statistics?.missing ||
                        results.missingClauses.length}
                    </span>
                  ...
                  <div className="flex flex-col items-center justify-center p-4 bg-warning/10 rounded-lg">
                    <span className="text-3xl font-bold text-warning">
                      {results.statistics?.corrupted ||
                        results.corruptedClauses.length}
                    </span>
                    <span className="text-sm font-semibold text-warning mt-1">
                      Corrupted
                    </span>
                  </div>
                </div>
                    ...
                    <div className="p-4 bg-muted/50 rounded-lg">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className="text-sm font-bold text-accent">
                          {results.statistics.completion_percentage.toFixed(1)}%
                        </span>
                      </div>
                    </div>
                  )}
                ...
                <div className="mt-6">
                  <Dialog
                    open={showClauseDetailsDialog}
                    onOpenChange={setShowClauseDetailsDialog}
                  >
                  ...
                  <p className="text-xs text-muted-foreground text-center mt-2">
                    ...
                    Click to see a detailed breakdown of all present, missing,
                    and corrupted clauses
                  </p>
                </div>
              </CardContent>
            </Card>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="text-lg flex items-center gap-2">
                    ...
                    <Badge
                      variant="outline"
                      className="ml-2 text-violet-600 border-violet-300"
                    >
                      {predictions.total_missing} suggestions
                    </Badge>
                    {predictions.source && (
                      <Badge variant="secondary" className="text-xs">
                        {predictions.source === "cache"
                          ? "⚡ cached"
                          : predictions.source === "llm"
                            ? "🤖 AI Predicted"
                            : predictions.source}
                      </Badge>
                    )}
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    <RefreshCw
                      className={`w-4 h-4 ${predictionsLoading ? "animate-spin" : ""}`}
                    />
                  </Button>
                  <Button
                    variant="ghost"
            ...
            {showPredictionsPanel && (
              <CardContent className="pt-0">
                <div className="space-y-3 max-h-[500px] overflow-y-auto pr-1">
                  {Object.entries(predictions.suggestions).map(
                    ([clauseKey, suggestion]) => {
                          ...
                          <div className="flex items-start justify-between mb-2">
                            <div className="flex-1">
                              <div className="flex items-center gap-2 flex-wrap">
                                ...
                                <Badge
                                  variant={
                                    suggestion.predictability === "FULL"
                                      ? "default"
                                      : "secondary"
                                  }
                                  className="text-xs"
                                ...
                                <Popover>
                                  <PopoverTrigger asChild>
                                    <button
                                      className={`text-xs font-semibold px-2 py-0.5 rounded-full ${confidenceBg} ${confidenceColor} hover:opacity-80 transition-opacity cursor-help`}
                                    >
                                  ...
                                  <PopoverContent className="w-64">
                                    <div className="space-y-2">
                                      ...
                                      <p className="text-xs text-muted-foreground">
                                        {suggestion.confidence >= 80
                                          ? "High confidence - This suggestion is highly reliable based on similar documents."
                                          : suggestion.confidence >= 50
                                            ? "Medium confidence - Review this suggestion carefully before accepting."
                              ...
                              <p className="text-xs text-muted-foreground mt-0.5">
                                Freq: {suggestion.frequency} | Position:{" "}
                                {suggestion.position}
                              </p>
                            </div>
                            {status !== "pending" && (
                          ...
                          {isEditing ? (
                            <div className="space-y-2">
                              <Textarea
                                ...
                                onChange={(e) =>
                                  setEditedSuggestionText(e.target.value)
                                }
                                rows={4}
                              ...
                              <div className="flex gap-2">
                                <Button
                                  size="sm"
                                  onClick={() =>
                                    handleSaveSuggestionEdit(clauseKey)
                                  }
                            ...
                            <div className="space-y-2 pt-2">
                              <div className="flex gap-2">
                                <Button
                                  size="sm"
                                  onClick={() =>
                                    handleAcceptSuggestion(clauseKey)
                                  }
                                ...
                                <Button
                                  ...
                                  variant="default"
                                  onClick={() =>
                                    handleEditSuggestion(clauseKey)
                                  }
                                ...
                                <Button
                                  ...
                                  variant="destructive"
                                  onClick={() =>
                                    handleRejectSuggestion(clauseKey)
                                  }
                                  className="flex-1 shadow-sm"
                              ...
                              <p className="text-xs text-muted-foreground text-center">
                                💡 Tip: Edit if you want to customize the
                                suggestion
                              </p>
                              <p className="text-xs text-amber-700 dark:text-amber-300 text-center">
                                ⚠ Sometimes the suggested insertion position may
                                be inaccurate — please verify the document after
                                accepting.
                              </p>
                            </div>
                          )}
              ...
              <div className="flex items-center gap-4 text-green-600">
                ...
                <div>
                  <p className="font-semibold text-base">
                    🎉 All clauses are present!
                  </p>
                  <p className="text-sm text-muted-foreground mt-1">
                    Your document contains all the required predictable clauses.
              ...
              <div className="flex flex-col items-center gap-4">
                ...
                <div className="text-center space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground max-w-md">
                    Analyzing document context and generating intelligent clause
                    suggestions based on legal precedents
                  </p>
                </div>
            ...
            <CardHeader className="flex flex-row items-center justify-between space-y-0">
              ...
              {analysisComplete && (
                <div className="flex items-center gap-2">
                  ...
                  {!isEditingDocument ? (
                    <Button size="sm" onClick={handleStartDocumentEdit}>
                      Edit
                    </Button>
                  ) : (
                    <>
                      <Button
                        size="sm"
                        onClick={handleDoneDocumentEdit}
                        className="bg-success"
                      >
                        Done
                      </Button>
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setViewMode(
                        viewMode === "review" ? "comparison" : "review",
                      )
                    }
                    className="ml-2"
                  >
                  ...
                  (isEditingDocument ? (
                    <div className="font-mono text-sm text-foreground bg-card p-2 rounded-lg shadow-sm">
                      <Textarea
                        ...
                        onChange={(e) =>
                          setModifiedDocumentText(e.target.value)
                        }
                        rows={24}
                ...
                {selectedIssue.type === "corrupted" && selectedIssue.data && (
                  <>
                    ...
                    <div className="p-4 bg-success/10 rounded-lg border-l-4 border-success">
                      <p className="text-xs font-semibold text-success uppercase mb-2">
                        💡 Suggestion
                      </p>
                      <p className="text-foreground">
                        {selectedIssue.data.suggestion}
                      </p>
                    </div>
                  </>
                )}
                ...
                {selectedIssue.type === "missing" && selectedIssue.data && (
                  <>
                    <div className="p-4 bg-destructive/10 rounded-lg border-l-4 border-destructive">
                      ...
                      <Badge
                        variant={getSeverityBadge(selectedIssue.data.severity)}
                      >
                        ⚠️ Missing Clause -{" "}
                        {selectedIssue.data.severity.toUpperCase()} PRIORITY
                      </Badge>
                    </div>
                    <div className="p-4 bg-muted rounded-lg">
                      <p className="text-xs font-semibold text-muted-foreground uppercase mb-2">
                    ...
                    <div className="p-4 bg-muted rounded-lg">
                      ...
                      <p className="text-foreground font-medium">
                        {selectedIssue.data.expectedLocation}
                      </p>
                    </div>
