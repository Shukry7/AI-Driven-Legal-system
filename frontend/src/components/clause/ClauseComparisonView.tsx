    ...
    return lines.map((line, idx) => {
      ...
      const hasAddedContent = changes.some((change) => {
        if (change.type === "added" || change.type === "corrected") {
          ...
          const snippet = change.newText.substring(
            0,
            Math.min(100, change.newText.length),
          );
          return (
            line.includes(snippet) || line.includes(change.name.toUpperCase())
          );
        }
        return false;
      });
      ...
      const hasRemovedContent = changes.some((change) => {
        if (change.type === "corrected" && change.originalText) {
          const snippet = change.originalText.substring(
            0,
            Math.min(100, change.originalText.length),
          );
  ...
  const handleDownloadComparison = async () => {
    try {
      ...
      changes.forEach((change, idx) => {
        comparisonText.push(
          `${idx + 1}. ${change.name} (${change.type.toUpperCase()})`,
        );
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={onComplete}>Complete & Finish</Button>
        </div>
      </div>
      ...
      {viewMode === "side-by-side" ? (
        <div className="grid grid-cols-2 gap-4">
          ...
          <Card>
            ...
            <CardContent>
              <div
                className="font-mono text-xs bg-muted/30 rounded-lg max-h-[600px] overflow-y-auto border p-2"
          ...
          <Card className="border-green-200 dark:border-green-800">
            ...
            <CardContent>
              <div
                className="font-mono text-xs bg-green-50/50 dark:bg-green-900/10 rounded-lg max-h-[600px] overflow-y-auto border border-green-200 dark:border-green-800 p-2"
                style={{ lineHeight: "1.2" }}
              >
        ...
        <Card>
          ...
          <CardContent>
            <div
              className="font-mono text-xs bg-muted/30 rounded-lg max-h-[700px] overflow-y-auto border p-2"
              style={{ lineHeight: "1.2" }}
            >
              {renderModifiedDocument(modifiedDocument)}
            </div>
