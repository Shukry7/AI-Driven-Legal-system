...
| Feature | Description |
|---|---|
| **Clause Detection** | Detects 28 predefined legal clauses via regex patterns validated on 450+ judgments. Flags clauses as Present, Missing, or Corrupted. |
| **AI Clause Prediction** | LLM-powered (OpenAI GPT-4o-mini) prediction of missing clauses with acceptance/rejection workflow and document finalisation. |
| **Document Translation** | Asynchronous English ↔ Sinhala ↔ Tamil translation with a built-in legal glossary of 9,057 terms. Supports PDF upload and raw text. |
...
3. `clause_detection_service` applies regex patterns to identify 28 clause types.
4. `corruption_detection_service` flags regions with potential text corruption.
5. If clause prediction is enabled, `clause_prediction_service` queries OpenAI (optionally with RAG context from ChromaDB) to suggest missing clauses.
...
├── legal_glossary.csv                 # 9,057 English / Sinhala / Tamil legal terms
│
...
| `OPENAI_API_KEY` | *(unset)* | OpenAI API key. Required when `CLAUSE_PREDICTION_MODE=openai` |
...
| Mode | Mechanism |
|---|---|
| `openai` | GPT-4o-mini with an engineered prompt; optionally augmented with RAG context |
| `manual` | Pre-defined template strings per clause type (no API key required) |
| `hybrid` | Tries OpenAI first; falls back to templates on error |
...
*Built for the Sri Lankan legal community — automating the analysis of Supreme Court civil judgments with AI.*
